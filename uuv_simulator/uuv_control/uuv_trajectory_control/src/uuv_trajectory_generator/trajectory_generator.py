...
class TrajectoryGenerator(object):
    ...
    * `stamped_pose_only` (*type:* `bool`, *default:* `False`): If `True`
        ...
        if not self._wp_interp_on:
            ...
            if len(self._points) > 1:
                if pnt.t <= self._points[-1].t:
                    self._logger.error('pnt.t <= self._points[-1].t and return false')
                    return False
            self._logger.info('add_trajectory_point')
    ...
    def get_max_time(self):
        ...
        points=self._points
        if points  is None and not self._wp_interp_on:
            if points is None:
                self._logger.error('self._points is None!!!!!!!!!!!')
            if  not self._wp_interp_on:    
                 self._logger.error('not self._wp_interp_on!!!!!!!!!!!')
            return None
        if self._wp_interp_on:
    ...
    def init_from_trajectory_message(self, msg):
        ...
        self._logger.info('msg.points.length()==='+str(len(msg.points)))
        ...
        for p_msg in msg.points:
            ...
            if last_t is None:
                ...
                if t <= last_t:
                    self._logger.info('t=='+str(t)+'   last t=='+str(last_t))
                    self._reset()
                    return False
...
## ========= Woods =========
# The error occurs when accessing the `self.object` attribute.
# The if-else condition to check if the `points` or `time` lists are None becomes ineffective.
# When receiving multiple replanned trajectories, the callback function that uses the `interpolate()` method
# tries to access the `points` or `time` lists, leading to unexpected calculations and resulting in an error.
        ...
        if not self._wp_interp_on:
            ...
            if points is None:
                self._logger.warning('points = None')
                return None
            if len(points) == 0:
                self._logger.warning('len(points) == 0')
                return None
            ...
            self._this_pnt.t = t
            # self._logger.warning(' points[0].t=='+str( points[0].t))
            # self._logger.warning(' current t'+str( t))


            if t <= points[0].t:
                self._logger.warning('t <= points[0].t')
                self._this_pnt.pos = deepcopy(points[0].pos)
                self._this_pnt.rotq = deepcopy(points[0].rotq)
                self._has_started = False
                self._is_finished = False
            if t >= points[-1].t:
                self._logger.warning('t >= points[-1].t')
                self._this_pnt.pos = deepcopy(points[-1].pos)
                self._this_pnt.rotq = deepcopy(points[-1].rotq)
                self._has_started=  True                                                   
                self._is_finished = True
            else:
                # self._logger.warning('=====t in traj=======')
                self._has_started = True
                self._is_finished = False
                idx = np.argmin(np.abs(time - t))
                if idx == 0:
                    self._this_pnt = deepcopy(points[0])
                else:
                    if t < points[idx].t:
                        p_this = points[idx]
                        p_last = points[idx - 1]
