...
from uuv_control_msgs.msg import Trajectory
...
##===================woods==================sub traj  directly
        self.input_traj_sub = rospy.Subscriber(
            'input_traj', Trajectory, self.traj_callback )
        self.trajectory=Trajectory()
        self.traj_start_time=rospy.Time()
        self.traj_has_start=False
        self.traj_has_finshed=False
    ...
    def get_point(self,t):
        ...
        ##TODO
        dt=0.35
        idx=int( (t - self.traj_start_time.to_sec())/dt )

        if idx< len(self.trajectory.points):
            return self.trajectory.points[idx]
        else:
    ...
    def odometry_callback(self, msg):
        ...
        # ===========woods============ Compute the desired position
        # Compute the desired position
        t = rospy.Time.now().to_sec()

        #des = self.local_planner.interpolate(t)
        if self.traj_has_start:
            des = self.get_point(t)
        else:
            des= TrajectoryPoint()
            des.pose.position=pos
            des.pose.orientation=quat
